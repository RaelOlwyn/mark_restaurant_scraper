...
url = 'https://www.mishlohim.co.il/menu/8178/'
...
restaurant_name_div = soup.find('div', attrs={'id': 'menu'})
for header in restaurant_name_div.findAll('h2'):
    span_array = header.findAll('span')
file_name = re.search(r'(.+)', span_array[1].text, flags=0).group(1).encode('UTF-8')
...
for section in sections:
	...
	for row in section.findAll('div', attrs={'class': 'row-item'}):
	    ...
	    for cell in row.findAll('p', attrs={'class': 'price'}):
	        text = cell.text.replace('&nbsp;', '').encode('UTF-8').replace('₪', '')
	        list_of_cells.append(text)
